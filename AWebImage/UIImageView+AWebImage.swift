...
public extension UIImageView {
    ...
    private class _AWImageLoaderPar :NSObject{
        ...
        init(url:NSURL, showLoading:Bool, imageProcess: AWebImageProcess?, completionBlock : AWImageLoaderCallback) {
            self.url = url
    ...
    public func aw_downloadImageURL(url:NSURL,
        ...
        loader.downloadImage(url, withImageProcess: imageProcess) { [weak self](image, url) in
            if showLoading {
                self?.aw_hideLoading()
            }
    ...
    public func aw_downloadImageURL_delay(url:NSURL,
        ...
        let loader = AWImageLoader()
        guard let fetch_key = loader.cacheKeyFromUrl(url, andImageProcess: imageProcess) else {
            return
        }
        if let cached_image = loader.imageFromFastCache(fetch_key) {
            self.aw_hideLoading()
